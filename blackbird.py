...
from jsonSchemas import searchDataSchema


DEFAULT_SEARCH_DATA_FILE_LOCATION = 'data.json'
...
async def findUsername(username, interfaceType, searchData):
    start_time = time.time()
    timeout = aiohttp.ClientTimeout(total=10)

    validate(instance=searchData, schema=searchDataSchema)
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        for u in searchData["sites"]:
            task = asyncio.ensure_future(makeRequest(
                session, u, username, interfaceType))
            tasks.append(task)

        results = await asyncio.gather(*tasks)
        now = datetime.now().strftime("%m/%d/%Y %H:%M:%S")
        executionTime = round(time.time() - start_time, 1)
        userJson = {"search-params": {"username": username, "sites-number": len(
            searchData['sites']), "date": now, "execution-time": executionTime}, "sites": []}
        for x in results:
...
async def makeRequest(session, u, username, interfaceType):
    url = u["url"].format(username=requote_uri(username))
    jsonBody = None
...
if __name__ == '__main__':
    ...
    dataArgumentHelp = f'''Location to the search data file (defaults to data.json) validated against the following json schema:
{json.dumps(searchDataSchema)}'''

    parser.add_argument('-d', '--data', action='store', dest='searchDataFileLocation',
                        required=False, default=DEFAULT_SEARCH_DATA_FILE_LOCATION, help=dataArgumentHelp)

    arguments = parser.parse_args()

    searchData = json.load(open(arguments.searchDataFileLocation))
